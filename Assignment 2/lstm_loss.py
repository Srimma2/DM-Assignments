...
#updates - use loss for Romney data, experiment hashtags, 
# experiment stpowords
...
def build_model():
    ...
    model.add(GRU(10,return_sequences=False,dropout_W=0.6,dropout_U=0.5)) #can also use LSTM
    #model.add(LSTM(15,return_sequences=False,W_regularizer=l22))
    #model.add(Dropout(0.2)) 
    #model.add(Dense(30, activation='relu'))used in GRU + FFNN 
    #model.add(Dropout(0.2))used in GRU + FFNN 
...
def build_CNN_model():
    ...
    model.add(Flatten())
    model.add(Dense(60,W_regularizer = l2 , activation='relu'))
...
for train,test in kf: #do the cross validation
    ...
    model = build_model()
    model.fit(x_train, y_train, nb_epoch=75, batch_size=64,verbose=0) #ep = 45 weight of 2
...
np.random.seed(1)
model = build_model()
history = model.fit(data, labels, nb_epoch=70, batch_size = 64 ,validation_split=0.2, shuffle=True)
